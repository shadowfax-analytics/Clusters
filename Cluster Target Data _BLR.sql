...
(
    SELECT
        d.city AS `City`,
d.operational_city As 'operational_city',

        d.cluster_name AS `Cluster Name`,
        a.cluster_id AS `Cluster ID`,
        MONTH(a.order_time) AS `Order Month`,
        CONCAT(month(a.order_time), "-", a.cluster_id) AS `or_primary_key`,
...
(
    SELECT
        ...
        CONCAT(month(a.attendancedate), "-", a.cluster_id) AS `at_primary_key`,
        CEIL(SUM(
            CASE
    ...
    WHERE
        ...
        AND month(attendancedate) > 9
    GROUP BY month(attendancedate), d.id
    LIMIT 10000000
) AS attendance_table
